...
public class MessagingController implements EventHandler<ActionEvent> {
    ...
    public  void onCreate() throws SQLException {
        ...
        for(int i=0;i< friends.size();i++){
            FirendUI x=new FirendUI(friends.get(i).getUserName());
            int finalI = i;
    ...
    private void ChangeChatBuddy(User user) throws SQLException {
        ...
        for(int i =0;i<messages.size();i++){
            ...
            if(messages.get(i).Sender.equals(Integer.toString(CurrentUser.GetCurrentUser().getId()))){
                ...
                obj.MessagingMain.getChildren().add(new IncomingMessage(messages.get(i).message));
            }
        }
        obj.x.setVvalue(1.0);
